...
//             children: [
//               const Text(' SIN APLICAR DEPENDENCY INVERSIÓN', style: TextStyle(fontSize: 25, fontWeight: FontWeight.bold),),
...
class DependencyInversion extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final NotificationHandler pushNotificationHandler =
        NotificationHandler(pushNotification);
    final NotificationHandler localNotificationHandler =
        NotificationHandler(localNotification);
    return MaterialApp(
      home: Scaffold(
        body: Center(
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              const Text('APLICANDO DEPENDENCY INVERSIÓN', style: TextStyle(fontSize: 25, fontWeight: FontWeight.bold),
              ),
              const SizedBox(height: 10),
              ElevatedButton(
                onPressed: () {
                  pushNotificationHandler
                      .handleNotification('¡Nueva notificación push!');
                },
                child: const Text('Recibir Notificación Push'),
              ),
              ElevatedButton(
                onPressed: () {
                  localNotificationHandler
                      .handleNotification('¡Nueva notificación local!');
                },
                child: const Text('Recibir Notificación Local'),
              ),
              const SizedBox(height: 20),
